...
$.fn.citySelect = function(options) {  
 	var defaults = {  
 		...
 		stval : ['省份/自治区','城市/地区','区/县','配送区域'],     //默认值
 		czemt : 'i',                        					  //存值元素
 	...
 	$.fn.appendlist = function(options){  //添加
 		...
 		for(var i = 0; i<appendArray.length;i++){
 			appList += '<li><a href="javascript:;" alt="'+appendArray[i]+'">'+appendArray[i]+'</a></li>';
 		}
 	...
 	$.fn.liClick = function(){
 			var $_liA = $('li' , _setId[0]),
 				...
 				$_liD = $('li' , _setId[3]),
 				indA,indB,indC,indD;
 			$('li' , _setId[0]).on('click' , function(){ //省点击事件
 			...
 			$(document).on('click' , _setId[1] + ' li' ,function(){ //市点击事件
 			...
 			$(document).on('click' , _setId[2] + ' li' , function(){ //区点击事件
 			...
 			$(document).on('click' , _setId[3] + ' li', function(){ //保险公司点击事件
